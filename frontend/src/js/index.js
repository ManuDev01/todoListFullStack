...
document.addEventListener("DOMContentLoaded", function () {
	const idGuardado = localStorage.getItem("usuarioActualId");
	usuarioActualId =
		idGuardado && idGuardado !== "null" ? Number(idGuardado) : null;
	console.log("Al cargar - usuarioActualId:", usuarioActualId);
	console.log(
		"Al cargar - localStorage:",
		localStorage.getItem("usuarioActualId")
	);
	renderTabs();
	renderTasks();
	acountClick();
...
document.addEventListener("DOMContentLoaded", function () {
	...
	renderTasks();
	acountClick();
	usuarioActualId = localStorage.getItem("usuarioActualId");
...
function agregarNuevaPestana() {
    console.log("Crear pestaña - usuarioActualId:", usuarioActualId);
    if (!usuarioActualId) {
        alert("Debes iniciar sesión para crear pestañas.");
        return;
    }
	mostrarModalNombrePestana(async function (nombrePestana) {
		const nuevaTab = {
			nombre: nombrePestana,
			idUsuario: usuarioActualId,
		};
		const tabCreada = await addTab(nuevaTab);
		if (tabCreada) {
			tabs.push({
				id: tabCreada.id,
				name: tabCreada.nombre,
				tasks: [],
			});
			activeTabId = tabCreada.id;
			renderTabs();
			renderTasks();
		} else {
			alert("Error al crear la pestaña");
		}
	});
}
...
function mostrarModalLogin() {
	...
	btnIniciar.onclick = async function () {
		...
		if (result && result.idusuario) {
			usuarioActualId = Number(result.idusuario);
			console.log("Login OK - usuarioActualId:", usuarioActualId);
